...
@implementation MLOrConstraint

- (id)init {
	self = [super init];
	if(self) {
		self.dict = [NSMutableDictionary dictionaryWithObject:[NSMutableArray arrayWithCapacity:20] forKey:@"or-query"];
	}

	return self;
}

+ (MLOrConstraint *)orConstraints:(MLConstraint *)firstConstraint, ... {
	...
	MLOrConstraint *constraint = [[[MLOrConstraint alloc] init] autorelease];
    
    [constraint.dict setObject:constraints forKey:@"or-query"];
